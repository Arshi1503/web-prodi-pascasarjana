    ...
    <div class="tw-card">
        ...
        <div class="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 lg:tw-grid-cols-3 xl:tw-grid-cols-4 tw-gap-6">
            ...
            <div class="tw-bg-[#b3b3b3] tw-shadow-md tw-rounded-lg tw-p-4 tw-flex tw-flex-col tw-items-center tw-relative tw-overflow-hidden">
                @if ($item->foto)
                <a href="{{ route('detail-dosen', ['id' => $item->id]) }}" class="tw-overflow-hidden tw-rounded-lg"><img class="tw-w-full tw-object-cover tw-rounded-lg tw-transition-transform lg:hover:tw-scale-110 hover:tw-scale-105 hover:tw-duration-700 active:tw-scale-90 hover:tw-ease-linear" src="{{ asset('storage/' . $item->foto) }}" alt="Dosen 1"></a>    
                @endif
                @if ($item->nama)
                <span class="tw-mt-6 tw-text-xl tw-text-white">{!! $item->nama !!}</span>    
                @endif
                @if ($item->status)
                <p class="tw-mt-2 tw-text-lg tw-text-gray-100 tw-text-center">{!! $item->status !!}</p>    
                @endif
            </div>
            @endforeach
        </div>
